...
app.post("/get-savedcontests", async (req, res) => {
  try {
    const { contestIds } = req.body;

    // Validate input
    const db = connection.useDb("coding-contests");
...
app.get("/api/get-solutions/:id", async (req, res, next) => {
  try {
...
app.get("/api/get-contests", async (req, res, next) => {
  try {
    const db = connection.useDb("coding-contests");
    ...
    const results = await Contests.aggregate(pipeline).toArray();
